# Prompts

Reusable task prompts. Use them, don't freestyle.

## Use

1. open prompt
2. fill `{{vars}}`
3. follow steps
4. run [`validate-repo.md`](validate-repo.md)

## Index

| task | prompt |
|------|--------|
| add source | [`add-source.md`](add-source.md) |
| bump source | [`bump-source.md`](bump-source.md) |
| extract items | [`extract-items.md`](extract-items.md) |
| classify | [`classify-conflict.md`](classify-conflict.md) |
| resolve | [`resolve-conflict.md`](resolve-conflict.md) |
| author eval | [`author-eval.md`](author-eval.md) |
| validate | [`validate-repo.md`](validate-repo.md) |
| cut release | [`cut-release.md`](cut-release.md) |

## Shape

Each prompt should have:

- purpose
- inputs
- preconditions
- steps
- output
- post-checks
